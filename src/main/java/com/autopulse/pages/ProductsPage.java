...
public class ProductsPage extends BasePage {
    ...
    public void searchProduct(String keyword) {
        ...
        // Don't click the button — send ENTER key directly
        // This bypasses ALL click/Actions/jsClick mechanisms
        // ENTER on a search box = same as clicking Search
        driver.findElement(searchBox)
                .sendKeys(org.openqa.selenium.Keys.RETURN);
        // Don't use waitForPageLoad() — with EAGER strategy
        // it returns before search results render.
        // Instead wait directly for the results header —
        // when THAT appears, results are definitely ready.
        wait.until(
                org.openqa.selenium.support.ui.ExpectedConditions
                        .visibilityOfElementLocated(searchResultsHeader)
        );
    ...
    public boolean isSearchResultsDisplayed() {
        return isDisplayed(allProductCards);
    }
    ...
    public ProductDetailPage viewProduct(int index) {
        ...
        // Extract the URL from the link and navigate directly
        // This completely bypasses click — no renderer timeout possible
        String productUrl = links.get(index).getAttribute("href");
        driver.get(productUrl);
        waitForPageLoad();

        System.out.println("👁️ Viewing product at index: "
                + index + " → " + productUrl);
