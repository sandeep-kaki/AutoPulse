...
//                chromeOptions.addArguments("--window-size=1920,1080");
                ...
                // ADD THESE — blocks popups and add
                chromeOptions.addArguments("--disable-popup-blocking");

//              // commented because it's causing crash "sesssion not created exception"
//                chromeOptions.addArguments("--block-new-web-contents");

                chromeOptions.addArguments("--disable-extensions");

                // Commented because these options actually used to run test cases in Docker/Linux CI environments. Now creating issues like browser crash
                chromeOptions.addArguments("--no-sandbox");
                chromeOptions.addArguments("--disable-dev-shm-usage");

                // ADD THIS — stops waiting for ads to finish loading
                ...
                if ("true".equals(ciEnv)) {
                    ...
                    // Required in cloud environments with no screen
                    chromeOptions.addArguments("--headless=new");
                    // Window size must be set manually in headless mode
                    // Without this — elements may not be found
                    // (page renders with 0x0 dimensions)
                    chromeOptions.addArguments("--window-size=1920,1080");
                    System.out.println("🤖 Running in CI mode — headless Chrome");
                } else {
                    System.out.println("💻 Running locally — visible Chrome");
                }
